...
| 対象期間     | 前回要約以降〜最新投稿まで                                                                                |
| 除外対象     | slack-summary の bot 自身が投稿したメッセージ（要約メッセージや処理中メッセージを含む）は要約対象から除外 |
| スキップ機能 | 前回の要約以降に新しいユーザーメッセージが追加されていない場合、要約生成をスキップし、スキップ理由を通知  |
...
- **要約対象の除外**: slack-summary の bot 自身が投稿したメッセージ（要約メッセージ、処理中メッセージなど）は要約対象から自動的に除外されます。
- **スキップ機能**: 前回の要約以降に新しいユーザーメッセージが追加されていない場合、要約生成をスキップし、「前回の要約以降、新しいメッセージが追加されていないため、要約をスキップします。」というメッセージが表示されます。
- **再要約**: 前回の要約以降に新しいユーザーメッセージがある場合のみ、要約が生成されます。
